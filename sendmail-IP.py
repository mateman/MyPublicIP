...
if not ( str(verIP) == str(message) and os.path.exists("myIP.txt") and os.path.isfile("myIP.txt")) :
    ...
    textFecha =  str("%s-%s-%s  " % (fecha.year, fecha.month, fecha.day))+str("%s:%s:%s" % (fecha.hour, fecha.month, fecha.second))
    msg['From'] = "unaCuenta@gmail.com"
    msg['To'] = "cuentaDestino@gmail.com"
    msg['Subject'] = "Nueva IP  "+ str(message)+"  " + textFecha
    ...
    arch.write(message)
    arch.close()
    if os.path.exists("listIP.txt") and os.path.isfile("listIP.txt") :
        arch = open("listIP.txt", 'a')
        arch.write(textFecha +"      "+ message +"\\n")
        arch.close()
